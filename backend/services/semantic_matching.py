...
class SemanticMatchingService:
    ...
    def _build_embedding_text(self, label: str, name: str | None, family: str | None) -> str:
        ...
        parts = [f"{label}: {clean_name}"]

        canonical_term = canonicalize_matching_term(clean_name)
        if canonical_term and canonical_term != clean_name.lower():
            parts.append(f"canonical concept: {canonical_term}")

        aliases = [
            alias for alias in expand_matching_aliases(clean_name)
        ...
        if clean_family and clean_family.lower() != "other":
            parts.append(f"family: {clean_family}")
